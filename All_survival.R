...
OS_cancer <- unique(survival_tcga_os_origin$cancer.type.abbreviation)
...
OS_cancer_1 <- filter(survival_tcga_os_origin,survival_tcga_os_origin$cancer.type.abbreviation==OS_cancer[i])
...
exp_sur <- merge(survival_tcga_os_origin,exp_survi,by='sample')
...
system.time(res3 <- mclapply(1:length(genes),mc.cores = 8,function(j){
  group = ifelse(exp_sur[,genes[j]]>median(exp_sur[,genes[j]]),"high","low")
  if(length(table(group))==1) return(NULL)
  surv =as.formula(paste('Surv(OS.time, OS)~', "group"))
  data = cbind(exp_sur[,1:2],group)
  data$group <- as.factor(data$group)
  data$group <- relevel(data$group,ref = 'low')
  x = survdiff(surv,data=data)
...
saveRDS(final_surivival_signi,'topsurvival.RData',compress = F)
